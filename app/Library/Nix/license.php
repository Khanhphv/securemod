...
class License
{
	...
	function __construct()
	{
		...
		$this->product->bits = 1024;
		$this->product->product_code = "jSccZO7IOlQ=";
		$this->product->public_exp  = "AQAB";
		$this->product->private_exp = 'L70zvyi8kA+Icfe8Yj4pgbZom+kOfXrO4hmOhSvzIJIrg6GUbK+P+ZiZApSNGjV0hnYS1f9gFE2gw3gY4u8momlx/QN6hQZ0Rk54o8CmvHu7hk/OYo5onVYh8drT+R2ZUtkxk+4nP5JyWSDUs5npO0IK1Er9374USHSgJC6te2E=';
		$this->product->modulus ='ANUJt8UW+E8kF5f4uEBU/fDeMDilkm88bJRimo+cBIqRlwYM5nwczve/VuZDa3YgQt20glHT1GpcwSkwOM9d2BTK50D/U4/W4GvVi40IVqZflDN8DzWwGiqYTJrf9x09fnzqg+IPaDaxQTwjN2Ri0sPWTr7hww1VTMRhX7sMP2rT';
	}
	
	public function init($bits, $product_code, $public_exp, $private_exp, $moduls) {
		$this->product->bits = $bits;
		$this->product->product_code = $product_code;
		$this->product->public_exp  = $public_exp;
		$this->product->private_exp = $private_exp;
		$this->product->modulus = $moduls;
	}
	
	public function DecodeChars($str)
	{
	...
	private function PackSerialData($data)
	{
		...
		if (!empty($data["data"]))
		{
			$userdata = $this->DecodeChars($data["data"]);
